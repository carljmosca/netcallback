 ...
 * protected host as well as the public host (running a PublicServer) and then
 * copies traffic between the two.
 *
 * @see PublicServer
 *
...
public class PrivateServer extends Thread implements SocketBridgeListener {
     ...
     */
    protected final DataOutputStream ostream;
    ...
    /**
     * Construct a private server receiving requests from the public host and
     * forwarding them to the private host.
     *
     * @param publicHost - the host where the PublicServer is running
     * @param publicPort - the port where the PublicServer is bound-to
     * @param tcpHost - the internal host that is the target of the redirection.
     * @param privatePort - the port in the internal host that is the target of
     * the redirection.
     * @param udpHost
     * @param udpPort
     ...
     * publicHost:publicPort could not be contacted, a communication error with
     * the PublicServer occured, or the remote host is unknown.
     *
     * @see PublicServer
     */
    public PrivateServer(String publicHost, int publicPort,
        ...
        //
        final PingThread pingThread = new PingThread(ostream);
        pingThread.start();
    }

    /**
     * Returns the DatagramSocket which should be used to communicate with the
     * protected UDP server so that the originating source can be identified.
     * <p>
     ...
     */
    protected synchronized DatagramSocket getDatagramSocket(final InetAddress srcAddress, final int srcPort)
    ...
    /**
     * While active, loop receiving requests on the established socket from the
     * public server. For each request, connect to the temporary socket on the
        ...
        try {
            while (active) {
                ...
                if (operation == CallbackProtocol.PING) {
                    ...
                    SocketBridge socketBridge = (SocketBridge) outPort2SocketBridgeMap.remove(new Integer(port));
                    ...
                    try {
                        ...
                        DatagramPacket packet = new DatagramPacket(data, length, udpHost, udpPort);
